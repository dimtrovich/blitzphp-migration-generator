...
use Dimtrovich\BlitzPHP\MigrationGenerator\Tokenizers\BaseColumnTokenizer;
...
class ColumnTokenizer extends BaseColumnTokenizer
{
    ...
    {
        $this->definition->setName($this->parseColumn($this->consume()));
    }
    ...
    private function consumeGenerated()
    {
        ...
        if (Text::startsWith($expression, '((') && Text::endsWith($expression, '))')) {
    ...
    {
        return Text::contains($this->columnDataType, ['char', 'text', 'set', 'enum']);
    }

    protected function isNumberType(): bool
    {
        return Text::contains($this->columnDataType, ['int', 'decimal', 'float', 'double']);
    }

    protected function isArrayType(): bool
    {
        return Text::contains($this->columnDataType, ['enum', 'set']);
    }

    protected function isBinaryType(): bool
    {
        return Text::contains($this->columnDataType, ['bit']);
    }

    public function getColumnDataType(): mixed
    {
